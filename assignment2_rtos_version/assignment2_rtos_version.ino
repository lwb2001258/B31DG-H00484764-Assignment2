...
SemaphoreHandle_t mutex,mutex0;
...
void setup() {
  ...
  xTaskCreatePinnedToCore(slackTimeUpdate, "slackTimeUpdate", 2048, NULL, 1, NULL, 0);
  ...
  xTaskCreatePinnedToCore(JobTask4, "Job4", 4096, NULL, 1, &jobHandles[3], 1);
  xTaskCreatePinnedToCore(JobTask5, "Job5", 4096, NULL, 1, &jobHandles[4], 0);
  xTaskCreatePinnedToCore(schedulerTask, "schedulerTask", 4096, NULL, 1, &jobHandles[5], 0);
...
void JobTask1(void *param) {
  while (1) {
    ...
    // Serial.println(micros()-now);
    // Serial.println("task 1 notify");
    xSemaphoreTake(mutex, portMAX_DELAY);
    // Serial.printf("Semaphore:");
    // Serial.println(micros()-now);
    // Serial.println("task 1 Semaphor");
    // ex_num++;
    // Serial.printf("1:");
...
void JobTask5(void *param) {
  while (1) {
    ...
    xSemaphoreTake(mutex0, portMAX_DELAY);
    // Serial.println("task 5 Semaphor");
    // ex_num++;
    // Serial.printf("5:");
    // Serial.println(ex_num);
